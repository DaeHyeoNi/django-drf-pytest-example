...
class TestBlogListView:
    ...
    def setup(self, db):
        ...
        self.user1 = User.objects.create_user(
            username="author1", email="author1@example.com", password="password123"
        )
    ...
    def test_blog_list_view(self):
        ...
        # 응답 데이터에서 게시물 찾기
        first_post = next(
            (post for post in response.data if post["slug"] == "first-post"), None
        )
        ...
        # 두 번째 게시물 검증
        second_post = next(
            (post for post in response.data if post["slug"] == "second-post"), None
        )
